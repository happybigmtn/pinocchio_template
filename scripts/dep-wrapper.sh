...
# Wrapper script to detect working directory for smart-deploy.sh
...
if [ -n "$1" ]; then
    # Pass arguments directly to smart-deploy.sh
...
if [[ "$(realpath --relative-to="$ROOT_DIR" "$CURRENT_DIR" 2>/dev/null || echo ".")" == "." ]]; then
    ...
    if [ ${#program_dirs[@]} -eq 1 ]; then
        ...
        if [ -n "$most_recent_dir" ] && [ -d "$most_recent_dir" ]; then
            ...
            echo "  bun dep my-program --category=basics"
...
# Set the detected directory and call smart-deploy.sh
ORIGINAL_PWD="$CURRENT_DIR" "$SCRIPT_DIR/smart-deploy.sh" "$@"
