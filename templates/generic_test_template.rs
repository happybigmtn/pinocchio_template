...
mod tests {
    ...
    pub mod test_utils {
        ...
        pub fn create_program_account(size: usize, owner: &Pubkey, lamports: u64) -> (Pubkey, AccountSharedData) {
            let pubkey = Pubkey::new_unique();
        ...
        /// 
        /// # Returns
        /// Vector of (Pubkey, AccountSharedData) tuples for common system accounts
        pub fn create_system_accounts() -> Vec<(Pubkey, AccountSharedData)> {
            let (system_program_id, system_account) = mollusk_svm::program::keyed_account_for_system_program();
            vec![
                (system_program_id, system_account.into()),
            ]
        }
        ...
        pub fn build_instruction(instruction_data: &[u8], accounts: Vec<AccountMeta>) -> Instruction {
        ...
        pub fn assert_instruction_error(
            ...
            match result.program_result {
                ProgramResult::Failure(error) => {
                    assert_eq!(error, expected_error, "Expected error {:?}, got {:?}", expected_error, error);
                }
                ProgramResult::Success => {
                    panic!("Expected instruction to fail with error {:?}, but it succeeded", expected_error);
                }
    ...
    fn test_create_account_data_example() {
        ...
        let (owner, owner_account) = test_utils::create_funded_account(1 * LAMPORTS_PER_SOL, &system_program);
        let (address_info_pubkey, address_info_account) = test_utils::create_funded_account(0, &system_program);
        ...
        assert_eq!(parsed_data.street, create_padded_array(b"Solana Street", 50));
    ...
    fn test_instruction_error_example() {
        ...
        let result = mollusk.process_instruction(
            &instruction,
            &[(owner, owner_account.into()), (system_program, system_account.into())],
        );
    ...
    fn test_account_validation_example() {
        let _mollusk = test_utils::create_mollusk("account_data_template");
